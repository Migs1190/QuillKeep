...
export namespace ExpoRouter {
  type StaticRoutes = `/` | `/(auth)` | `/(tabs)` | `/_sitemap` | `/home` | `/notifications` | `/profile` | `/recover` | `/settings` | `/sign-in` | `/sign-up`;
  type DynamicRoutes<T extends string> = `/book/${SingleRoutePart<T>}` | `/search/${SingleRoutePart<T>}`;
  type DynamicRouteTemplate = `/book/[title]` | `/search/[query]`;
